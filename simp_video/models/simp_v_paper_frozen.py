...
def train_simp_v(
    ...
    for epoch in range(max_epochs):
        ...
        for i in range(0, data.shape[0], 8):
            ...
            coeff_reg = coeff_reg_weight * torch.mean((coeffs_pred - theta_true_t.unsqueeze(0)) ** 2)
            l1_reg = l1_weight * torch.mean(torch.sum(torch.abs(coeffs_pred), dim=-1))
            total = loss + coeff_reg + l1_reg
            total.backward()
